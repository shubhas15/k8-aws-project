# Cloud-Native Web Application with Kubernetes
...
- **Cloud technology**: We are using AWS EKS service to deploy our application. 
...
Create EKS cluster with NodeGroup (3 nodes of t3.medium instance type)
Create EC2 Instance t3.micro (Optional) and attach below IAM role to the instance
...
```
git clone https://github.com/shubhas15/kubernetes-app-db-project.git
```

**Create dev-voting Namespace**
```
kubectl create ns dev-voting

kubectl config set-context --current --namespace dev-voting
```
...
After the application has loaded successfully, by clicking on several of the **+1** buttons, this will generate a traffic which will be sent back to the API via the API's assigned ELB.
...
In this Project, we deployed a cloud native application into EKS. Once deployed and its up and running, use your local workstation's browser to test out the application. Later confirmed that our activity within the application generated data which was captured and recorded successfully within the MongoDB ReplicaSet back end within the cluster.
